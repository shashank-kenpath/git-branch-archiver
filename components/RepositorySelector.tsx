...
import { AlertCircle, Github, Search, User, Building2, ChevronDown } from "lucide-react"
                ...
                {organizations.length === 0 ? (
                  ...
                  <Accordion type="multiple" className="w-full">
                    {organizations.map((org) => {
                      const orgRepos = getOrganizationRepos(org.login);
                      return (
                        <AccordionItem key={org.login} value={org.login}>
                          <AccordionTrigger className="hover:no-underline">
                            <div className="flex items-center gap-2">
                              <Building2 className="h-4 w-4" />
                              <span className="font-medium">{org.login}</span>
                              <span className="text-sm text-muted-foreground ml-2">
                                ({orgRepos.length} repositories)
                              </span>
                            </div>
                          </AccordionTrigger>
                          <AccordionContent>
